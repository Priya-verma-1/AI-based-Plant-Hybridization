...
// const { computeHybridTraits, deriveHybridName } = require('../utils/dataTransform')
...
//     const payload = {
//       plantA: {
//         name: plantA.plant_name, plant_name: plantA.plant_name,
//         height_cm: plantA.height_cm, leaf_shape: plantA.leaf_shape,
//         flower_color: plantA.flower_color, climate: plantA.climate,
//         resistance: plantA.resistance, growth_days: plantA.growth_days,
//         yield_level: plantA.yield_level, family: plantA.family || '',
//       },
//       plantB: {
//         name: plantB.plant_name, plant_name: plantB.plant_name,
//         height_cm: plantB.height_cm, leaf_shape: plantB.leaf_shape,
//         flower_color: plantB.flower_color, climate: plantB.climate,
//         resistance: plantB.resistance, growth_days: plantB.growth_days,
//         yield_level: plantB.yield_level, family: plantB.family || '',
//       },
//     }

//     try {
//       console.log(`🤖  Calling ML API → ${ML_API_URL}/predict`)
//       const { data } = await mlAxios.post('/predict', payload)

//       const rawTraits  = data.traits || data.predicted_traits || {}
//       const hybridName = data.predicted_hybrid || deriveHybridName(plantA.plant_name, plantB.plant_name)
...
//     } catch (err) {
//       const reason = err.code === 'ECONNREFUSED' ? 'unreachable' :
//                      err.response?.status === 503 ? 'not trained' :
//                      err.code === 'ECONNABORTED'  ? 'timeout' : err.message
//       console.warn(`⚠️   ML API ${reason}. Using rule-based fallback.`)
//       return this._fallback(plantA, plantB)
//     }
//   }

//   _fallback(plantA, plantB) {
//     const traits     = computeHybridTraits(plantA, plantB)
//     const hybridName = deriveHybridName(plantA.plant_name, plantB.plant_name)
//     console.log(`📐  Rule-based fallback: ${hybridName}`)
//     return { traits, hybridName, source: 'rule_based', confidence: null }
//   }

//   async healthCheck() {
//     try {
//       const { data } = await mlAxios.get('/health', { timeout: 3000 })
//       return { status: data.status || 'ok', models_trained: data.models_trained || false, url: ML_API_URL }
//     } catch {
//       return { status: 'unreachable', url: ML_API_URL }
//     }
//   }
// }
