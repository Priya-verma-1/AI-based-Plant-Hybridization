...
//    * @param {string} opts.source         - 'ml_api' | 'rule_based' | 'fallback'
//    * @returns {PredictionHistory}        - Saved Mongoose document
//    */
//   async save({ plantA, plantB, traits, hybridName, source }) {
...
//       prediction_source:source || 'rule_based',
...
class HistoryService {
   ...
   * @returns {PredictionHistory}        - Saved Mongoose document
   */
  async save({ plantA, plantB, traits, hybridName, source, traitDescriptions }) {
    const record = await PredictionHistory.create({
      plantA:           plantA.plant_name,
      plantB:           plantB.plant_name,
      plantA_data:      plantA,
